...
(function () {
  ...
  const parseChallengeText = (raw) => {
    ...
    lines.forEach(line => {
      ...
      // Shiny
      if (/^shiny:/i.test(trimmed)) {
        const shinyValue = trimmed.replace(/^shiny:/i, "").trim().toLowerCase();
        current.shiny = ["yes", "true"].includes(shinyValue);
        return;
      }
  ...
  const openPlayerSelection = (challenge, parseResult) => {
    ...
    parseResult.playerTeam.forEach((mon, idx) => {
      ...
      const sprite = mon.shiny ? `img/pkmn/shiny/${mon.id}.png` : `img/pkmn/sprite/${mon.id}.png`;
  ...
  const renderChallengeCard = (challenge, options = {}) => {
    ...
    if (allowExport) {
      actions.appendChild(createButton("Export", () => handleExport(challenge)));
    }
